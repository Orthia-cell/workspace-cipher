#!/usr/bin/env python3
"""
Analog-only perplexity test. Compares against known digital baseline.
"""
...
DIGITAL_BASELINE = 8.64  # Known from prior test

# Use fewer texts to reduce memory pressure
SAMPLE_TEXTS = [
...
def calculate_perplexity(model, tokenizer, texts, max_length=64):
    """Calculate perplexity with reduced max_length to save memory."""
    model.eval()
    total_loss = 0.0
    total_tokens = 0
...
def main():
    ...
    print('=== TinyLlama Analog Perplexity Test ===')
    print(f'Target layers: {target_layers}')
    print(f'PCM noise η: {noise_eta}')
    print(f'Text samples: {len(SAMPLE_TEXTS)} (reduced for memory)')
    print(f'Known digital baseline: {DIGITAL_BASELINE}')
    print()
