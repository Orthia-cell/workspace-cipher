...
# Sample texts for perplexity evaluation (same style as real activation capture)
SAMPLE_TEXTS = [
...
def create_analog_matrix(original_module, noise_eta=0.038):
    """Create an analog version of a Linear module."""
...
def map_layers_to_analog(model, target_layers, noise_eta=0.038):
    """Map all matrices in target_layers to analog tiles."""
...
def main():
    ...
    noise_eta = 0.038  # IBM PCM noise level
    ...
    # Test 1: All-digital baseline
    print('\n--- Test 1: All-Digital Baseline ---')
    print('Loading model (digital)...')
    model_digital = AutoModelForCausalLM.from_pretrained(
        MODEL_ID,
    ...
    ppl_digital = calculate_perplexity(model_digital, tokenizer, SAMPLE_TEXTS)
    ...
    print('\n--- Test 2: Analog Layers 18-21 ---')
    print('Loading model (for analog mapping)...')
    model_analog = AutoModelForCausalLM.from_pretrained(
        MODEL_ID,
        torch_dtype=torch.float16,
        device_map='cpu',
        low_cpu_mem_usage=True
    ...
    # Map layers to analog
    model_analog = map_layers_to_analog(model_analog, target_layers, noise_eta)
    
    print('Calculating analog perplexity...')
    ppl_analog = calculate_perplexity(model_analog, tokenizer, SAMPLE_TEXTS)
