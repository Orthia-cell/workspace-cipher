#!/usr/bin/env python3
"""
Selective mapping test: only map k_proj and o_proj (PCM-insensitive matrices).
Hypothesis: Degradation drops below 10%.
"""
...
def map_selective_to_analog(model, target_layers, noise_eta=0.038):
    """Map ONLY k_proj and o_proj to analog (PCM-insensitive matrices)."""
    for layer_idx in target_layers:
        layer = model.model.layers[layer_idx]
        print(f'  Selective mapping layer {layer_idx}: k_proj + o_proj only...')
    ...
    print(f'Mapped {len(target_layers)} layers selectively (k_proj + o_proj only, η={noise_eta})')
    return model
...
def main():
    ...
    print('=== SmolLM2-135M Selective Mapping Test (k_proj + o_proj only) ===')
    ...
    # Test 1: Digital baseline (reuse known value 11.19 to save time, or recalculate)
    ...
    # Test 2: Selective analog mapping
    print('\n--- Test 2: Selective Analog (k_proj + o_proj only) ---')
    print('Loading model...')
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_ID,
        torch_dtype=torch.float16,
    ...
    model = map_selective_to_analog(model, target_layers, noise_eta)
    
    print('Calculating selective analog perplexity...')
    ppl_analog = calculate_perplexity(model, tokenizer, SAMPLE_TEXTS)
    print(f'Selective Analog Perplexity: {ppl_analog:.2f}')
    ...
    if pct_degradation < 5:
        verdict = 'GREEN — Excellent analog viability!'
    elif pct_degradation < 10:
        verdict = 'GREEN — Analog is viable!'
    elif pct_degradation < 20:
        verdict = 'YELLOW — Marginal, acceptable for many use cases'
    else:
    ...
    results = {
        ...
        'ratio': float(ratio),
        'verdict': verdict,
        'comparison_to_full_mapping': 'Full 7-matrix mapping was +16.5%, selective 2-matrix is expected lower',
    }
    
    output_path = f'{OUTPUT_DIR}/perplexity_selective_k_o_proj.json'
